                    ...
                    <div>
                        {u.followed
                            ? <button disabled={props.followingInProgress.some(id => id === u.id)} className={styles.followBtn} onClick={() => { 
                                ...
                                axios
                                    .delete(`https://social-network.samuraijs.com/api/1.0/follow/${u.id}`, {withCredentials: true, headers: {"API-KEY" : "f6edd1e2-78d4-4b10-837a-975d6d93f70e"}})
                                    .then(response => {
                            ...
                            : <button disabled={props.followingInProgress.some(id => id === u.id)} className={styles.followBtn} onClick={() => { 
                                props.toggleFollowingProgress(true, u.id)
                                axios
                                    .post(`https://social-network.samuraijs.com/api/1.0/follow/${u.id}`,{}, {withCredentials: true, headers: {"API-KEY" : "f6edd1e2-78d4-4b10-837a-975d6d93f70e"}})
                                    .then(response => {
                                        if(response.data.resultCode === 0){
                                            props.follow(u.id) 
                                            props.toggleFollowingProgress(false, u.id)
